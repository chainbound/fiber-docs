...
## Direct Peering
Just like with [FiberGuard](/docs/fiber-guard), we offer a direct peering connection on the consensus layer for boosting blocks. This means that all blocks we receive on said connection will be boosted on our network.

The requirements and setup procedure are the same as with FiberGuard, you can read them [here](/docs/fiber-guard#requirements).
