...
To check this latency, just run:

```bash
ping beta.fiberapi.io
```

from the same machine as your client.
...
protobuf / gRPC files and a compiler for your language. Documentation for different gRPC language
...
For examples of how to actually use it (API authentication), please refer to the already implemented packages.
:::
...
Here is how you can subscribe to the new blocks stream in the different packages:
...
- `SendTransactionSequence`: takes an array of signed transactions to be sent in sequence
- `SendRawTransactionSequence`: takes an array of signed, RLP encoded transactions to be sent in sequence

:::info
The `SendTransactionSequence` and `SendRawTransactionSequence` methods don't guarantee that the transactions
will land in order onchain. They will be sent in sequence, but the final ordering will be determined by the
